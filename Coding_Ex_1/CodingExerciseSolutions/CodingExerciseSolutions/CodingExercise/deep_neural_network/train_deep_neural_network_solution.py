...
def get_model_wine(input_dimension):
    ...
    if args.network_type == "sequential":
        ...
        model.add(tf.keras.layers.Dense(64, activation='relu', input_shape=(input_dimension,)))
...
def get_model_banking(input_dimension):
    ...
    if args.network_type == "sequential":
        # TODO define a Sequential model (https://www.tensorflow.org/api_docs/python/tf/keras/Sequential)
        model = tf.keras.Sequential([
            tf.keras.layers.Dense(100, activation=tf.nn.sigmoid, input_shape=(input_dimension,)),  # input shape required
    ...
    if args.network_type == "functional":
        # TODO define a Functional model (https://www.tensorflow.org/guide/keras/functional)
        inputs = tf.keras.Input(shape=(input_dimension,))
        dense1 = tf.keras.layers.Dense(100, activation="sigmoid")(inputs)
        dense2 = tf.keras.layers.Dense(100, activation="sigmoid")(dense1)
        dense3 = tf.keras.layers.Dense(10, activation="sigmoid")(dense2)
        outputs = tf.keras.layers.Dense(2)(dense3)

        model = tf.keras.Model(inputs=inputs, outputs=outputs, name="banking_model")
        return model
...
def get_model_iris(input_dimension):
    # Define a model with the following layers:
    #  1. Dense layer (input_dimension -> 10, relu activation)
    #  2. Dense layer (10 -> 10, relu activation)
    #  3. Dense layer (10 -> 3)
    ...
    if args.network_type == "sequential":
        # TODO define a Sequential model (https://www.tensorflow.org/api_docs/python/tf/keras/Sequential)
        model = tf.keras.Sequential([
            tf.keras.layers.Dense(10, activation=tf.nn.relu, input_shape=(input_dimension,)),  # input shape required
...
if __name__ == '__main__':
    ...
    predict_dataset = pd.read_csv(path_prediction, delimiter=",")
    predict_dataset = predict_dataset.values
    ...
    if args.implementation == "detail":
        ...
        for epoch in range(args.num_epochs):
            ...
            for x, y in train_dataset:
                ...
                epoch_accuracy.update_state(y, model(x, training=True)) # we only have to use training=True when layers
                # differentiate between training and testing (eg dropout)
        ...
        for i, pred in enumerate(predictions):
            if args.loss == "CategoricalCrossEntropy":
                ...
                p = tf.nn.softmax(pred)[class_idx]
